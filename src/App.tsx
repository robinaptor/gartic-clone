...
// --- OPTION 1 : CONFIGURATION AUTOMATIQUE (CANVAS) ---
// const firebaseConfig = JSON.parse(__firebase_config);
// const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';

// --- OPTION 2 : CONFIGURATION MANUELLE (LOCAL) ---
...
const appId = 'gartic-party-local'; // Vous pouvez laisser ce nom ou le changer
...
// --- TYPES ---
type Phase = 'LOBBY' | 'WRITE_START' | 'DRAW' | 'GUESS' | 'VOTE' | 'PODIUM' | 'RESULTS';
...
// --- COMPOSANT DESSIN ---
const DrawingCanvas = ({ initialImage, onSave, isReadOnly = false }: { initialImage?: string | null, onSave: (data: string) => void, isReadOnly?: boolean }) => {
            ...
            <div className="bg-white border-4 border-black rounded-2xl p-4 shadow-hard flex flex-row md:flex-col gap-4 w-full md:w-auto items-center justify-center">
                <div className="flex md:flex-col gap-2">
                    <button 
                        ...
                        className={`p-3 rounded-xl border-2 border-black transition-transform ${!isEraser ? 'bg-purple-500 text-white -translate-y-1 shadow-hard-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                    >
                        <Pencil size={24} />
                    </button>
                    <button 
                ...
                <div className="flex md:flex-col gap-2 items-center">
                    {BRUSH_SIZES.map(size => (
                        <button 
                            ...
                            className={`rounded-full bg-black transition-all ${brushSize === size ? 'ring-4 ring-yellow-400 scale-110' : 'opacity-30 hover:opacity-60'}`}
                ...
                <div className="grid grid-cols-5 md:grid-cols-2 gap-2">
                    {DRAW_COLORS.map((c) => (
                        <button
                            ...
                            className={`w-8 h-8 rounded-full border-2 border-black transition-transform hover:scale-110 shadow-sm ${color === c && !isEraser ? 'ring-4 ring-yellow-400 scale-110 z-10' : ''}`}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(roomRef, (snap) => {
      if (snap.exists()) setCurrentRoom(snap.data() as RoomData);
  ...
  const castVote = async (targetChainOwnerId: string, stepIndex: number) => {
    ...
    if (updatedSteps[stepIndex]) {
        updatedSteps[stepIndex].votes = (updatedSteps[stepIndex].votes || 0) + 1;
    }
    const roomRef = doc(db, 'artifacts', appId, 'public', 'data', 'gartic_rooms', roomCode);
    await updateDoc(roomRef, {
  ...
  useEffect(() => {
    ...
    if (allReady && !['LOBBY', 'VOTE', 'PODIUM', 'RESULTS'].includes(currentRoom.phase)) {
      if (currentRoom.round + 1 >= currentRoom.players.length) {
         const roomRef = doc(db, 'artifacts', appId, 'public', 'data', 'gartic_rooms', roomCode);
         updateDoc(roomRef, {
             phase: 'VOTE',
         ...
         const nextPhase = (currentRoom.round + 1) % 2 === 0 ? 'GUESS' : 'DRAW'; 
         const roomRef = doc(db, 'artifacts', appId, 'public', 'data', 'gartic_rooms', roomCode);
         setTimeout(() => {
             updateDoc(roomRef, {
                 round: increment(1),
  ...
  const renderContent = () => {
        ...
        <div className="min-h-screen bg-pattern flex items-center justify-center p-4 font-sans">
            ...
            <FunCard className="w-full max-w-md text-center border-black border-4 shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] transform rotate-1 hover:rotate-0 transition-transform duration-300">
                <div className="mb-8 relative">
                    <h1 className="text-6xl font-black text-yellow-400 tracking-tighter uppercase drop-shadow-[4px_4px_0_rgba(0,0,0,1)] stroke-black" style={{WebkitTextStroke: '2px black'}}>
                        Gartic Clone
                    </h1>
                    <div className="absolute -top-6 -right-6 animate-bounce">
                        <Pencil size={48} className="text-purple-500 fill-purple-300 drop-shadow-md" />
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    <div className="text-left space-y-2">
                        ...
                        <input 
                            ...
                            className="w-full px-4 py-3 rounded-xl border-4 border-black font-bold text-xl focus:outline-none focus:shadow-hard transition-all bg-gray-50 placeholder-gray-400" 
                            placeholder="SuperArtiste..."
                        />
                    </div>

                    <div className="flex flex-col gap-4 pt-2">
                        <FunButton onClick={createRoom} disabled={loading || !playerName} color="purple" icon={Palette}>
                            Créer une Party
                        </FunButton>
                        
                        <div className="flex items-center gap-4">
                            <div className="h-1 flex-1 bg-black rounded-full"></div>
                            <span className="font-black text-gray-400">OU</span>
                            <div className="h-1 flex-1 bg-black rounded-full"></div>
                        </div>
                        ...
                        <div className="flex gap-2">
                            <input 
                                ...
                                className="flex-1 px-4 py-3 rounded-xl border-4 border-black font-mono text-center font-black text-xl uppercase tracking-widest focus:outline-none focus:shadow-hard" 
                                placeholder="CODE" 
                                maxLength={6}
                            />
                            <FunButton onClick={joinRoom} disabled={loading || !playerName || !joinCode} color="green">
                                GO
                            </FunButton>
                        </div>
                    </div>
                    <div className="mt-4 bg-yellow-100 border-2 border-black rounded-lg p-2 text-xs font-bold text-yellow-800 transform -rotate-2">
                        Astuce : Ouvre un autre onglet pour jouer avec toi-même ! 🤪
                    </div>
                </div>
            </FunCard>
        </div>
        ...
        return (
            <div className="min-h-screen bg-pattern p-6 font-sans">
                ...
                <div className="max-w-5xl mx-auto space-y-8">
                    <div className="bg-white border-4 border-black rounded-3xl p-6 shadow-hard-lg flex flex-col md:flex-row justify-between items-center gap-6">
                        ...
                        <div className="flex flex-col items-center gap-2">
                             {me?.isHost && currentRoom.players.length > 1 ? (
                                <FunButton onClick={startGame} color="green" icon={Play} className="animate-pop">
                                    Lancer la partie !
                                </FunButton>
                             ) : me?.isHost ? (
                                <div className="px-4 py-2 bg-yellow-100 border-2 border-black rounded-lg font-bold text-yellow-800 text-center animate-pulse">
                                    Attends des potes...
                                </div>
                             ) : (
                                <div className="text-purple-600 font-black animate-pulse text-xl">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                        ...
                        {[...Array(Math.max(0, 8 - currentRoom.players.length))].map((_, i) => (
                             <div key={i} className="border-4 border-dashed border-black/20 rounded-2xl aspect-square flex items-center justify-center">
        ...
        return (
            <div className="min-h-screen bg-pattern flex flex-col items-center justify-center p-4 text-center space-y-8">
                ...
                <FunCard className="animate-float flex flex-col items-center p-12">
                    ...
                    <p className="text-gray-500 font-bold text-xl">On attend les trainards...</p>
                </FunCard>
                <div className="flex gap-3 bg-black/20 p-4 rounded-2xl backdrop-blur-sm">
                    {currentRoom.players.map(p => (
                         <div 
                            key={p.uid} 
                            className={`w-4 h-4 rounded-full border-2 border-white transition-all duration-300 ${p.isReady ? 'bg-green-400 scale-125' : 'bg-gray-400'}`} 
                         />
                    ))}
                </div>
            </div>
            ...
            <div className="min-h-screen bg-yellow-400 flex items-center justify-center p-4 font-sans bg-pattern" style={{backgroundColor: '#facc15'}}>
                ...
                <FunCard title="Round 1" className="w-full max-w-3xl text-center space-y-6 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
                    <div className="space-y-2">
                        <h2 className="text-4xl font-black text-black uppercase">Invente une situation !</h2>
                        <p className="text-gray-500 font-bold text-lg">Sois créatif, bizarre ou juste stupide.</p>
                    </div>
                    ...
                    <div className="relative">
                        <textarea 
                            ...
                            className="w-full h-40 p-6 text-3xl text-center font-black border-4 border-black rounded-2xl focus:outline-none focus:shadow-hard resize-none bg-gray-50 leading-normal"
                            placeholder="Un pingouin qui mange une raclette..."
                            maxLength={80}
                        />
                        <div className="absolute bottom-4 right-4 text-xs font-bold text-gray-400">
                            {inputContent.length}/80
                        </div>
                    </div>
            ...
            <div className="min-h-screen bg-blue-500 flex flex-col font-sans">
                ...
                <div className="bg-white border-b-4 border-black p-4 shadow-md z-10 flex justify-between items-center">
                    <div className="flex items-center gap-4">
                        ...
                        <h2 className="text-xl md:text-2xl font-black truncate max-w-[50vw] text-purple-600">
                            "{prevStep.content}"
                        </h2>
                    </div>
                    <FunButton onClick={submitContent} disabled={!inputContent} color="green" className="px-4 py-2 text-sm">
                        J'ai fini !
                    </FunButton>
                </div>
            ...
            <div className="min-h-screen bg-purple-600 flex items-center justify-center p-4 font-sans bg-pattern">
                ...
                <FunCard className="w-full max-w-5xl flex flex-col md:flex-row overflow-hidden p-0 border-0">
                    <div className="w-full md:w-2/3 bg-gray-100 border-b-4 md:border-b-0 md:border-r-4 border-black flex flex-col relative">
                        <div className="absolute top-4 left-4 z-10 bg-yellow-400 border-2 border-black px-3 py-1 rounded-full font-black text-xs shadow-hard-sm">
                            DESSIN DE {prevStep.authorName.toUpperCase()}
                        </div>
                        <div className="flex-1 flex items-center justify-center bg-white p-4">
                             <img src={prevStep.content} alt="Guess this" className="max-w-full max-h-[50vh] object-contain drop-shadow-lg transform rotate-1" />
                        </div>
                    </div>

                    <div className="w-full md:w-1/3 p-8 flex flex-col justify-center space-y-6 bg-white">
                         <div className="text-center space-y-2">
                             <h2 className="text-3xl font-black uppercase leading-none">C'est quoi ce truc ?</h2>
                             <p className="text-gray-500 font-bold text-sm">Décris ce chef-d'œuvre.</p>
                         </div>

                         <textarea 
                            value={inputContent}
                            onChange={(e) => setInputContent(e.target.value)}
                            className="w-full h-32 p-4 text-xl text-center font-bold border-4 border-black rounded-xl focus:outline-none focus:shadow-hard bg-blue-50 resize-none"
                            placeholder="Je pense que c'est..."
                        />

                        <FunButton onClick={submitContent} disabled={!inputContent.trim()} color="purple" icon={CheckCircle}>
                            Valider
                        </FunButton>
                    </div>
                </FunCard>
    ...
    if (currentRoom.phase === 'VOTE') {
        ...
        if (me?.hasVoted) {
             return (
                <div className="min-h-screen bg-indigo-900 flex flex-col items-center justify-center p-4 text-center text-white space-y-6 bg-pattern">
                    <GlobalStyles />
                    <div className="w-32 h-32 bg-yellow-400 rounded-full border-4 border-black flex items-center justify-center animate-bounce shadow-hard-lg">
                        <Star className="text-black fill-white" size={64} />
                    </div>
                    <div>
                        <h2 className="text-4xl font-black uppercase text-yellow-400 drop-shadow-md">Vote Enregistré !</h2>
                        <p className="text-white/80 font-bold mt-2 text-xl">Suspense...</p>
                    </div>
                    <div className="flex flex-wrap gap-3 justify-center mt-8 p-4 bg-black/30 rounded-xl backdrop-blur-sm">
                        {currentRoom.players.map(p => (
                            <div key={p.uid} title={p.name} className={`w-4 h-4 rounded-full border-2 border-black transition-all ${p.hasVoted ? 'bg-green-400 scale-125' : 'bg-gray-600'}`} />
                        ))}
                    </div>
                </div>
            );
        ...
        Object.entries(currentRoom.chains).forEach(([ownerId, chain]) => {
            chain.steps.forEach((step, idx) => {
                if (step.type === 'DRAWING') {
                    allDrawings.push({ chainOwnerId: ownerId, stepIndex: idx, step });
                }
            });
        });
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="text-center mb-12 pt-8 space-y-4">
                        ...
                        <p className="text-white font-bold text-xl bg-black/50 inline-block px-6 py-2 rounded-full">
                            Vote pour le dessin le plus drôle / beau / bizarre !
                        </p>
                    </div>
                        ...
                        {allDrawings.map((item, i) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <p className="font-handwriting text-gray-500 font-bold text-sm text-left transform rotate-1">
                                        #{i + 1}
                                    </p>

                                    {isMine && (
                                        <div className="absolute inset-0 flex items-center justify-center bg-black/10 font-black text-red-600 text-2xl uppercase tracking-widest -rotate-12 border-4 border-red-600 m-8 rounded-xl bg-white/80">
                                            C'est toi !
                                        </div>
                                    )}
                                    {!isMine && (
                                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                            <Star className="text-yellow-400 fill-yellow-400 drop-shadow-lg animate-pop" size={80} />
                                        </div>
                                    )}
                                </button>
                            )
    ...
    if (currentRoom.phase === 'PODIUM') {
        return <PodiumView room={currentRoom} onResults={() => {
             const roomRef = doc(db, 'artifacts', appId, 'public', 'data', 'gartic_rooms', roomCode);
             updateDoc(roomRef, { phase: 'RESULTS' });
        }} />;
    }
...
// --- PODIUM VIEW (CORRIGÉ & INCASSABLE) ---
        ...
        Object.values(room.chains).forEach(chain => {
            if (chain?.steps) {
                chain.steps.forEach(step => {
                    if (step && step.type === 'DRAWING' && (step.votes || 0) > 0) {
                        if (step.authorId) {
                            scores[step.authorId] = (scores[step.authorId] || 0) + (step.votes || 0);
                        }
                    }
                });
            }
    ...
    return (
        <div className="min-h-screen bg-purple-900 flex flex-col items-center justify-center p-4 overflow-hidden relative font-sans bg-pattern">
            ...
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                 {[...Array(50)].map((_, i) => (
                    <div key={i} className="absolute w-4 h-4 bg-yellow-400 animate-float" 
                         style={{ 
                             left: `${Math.random() * 100}%`, 
                             top: `${Math.random() * 100}%`, 
                             animationDelay: `${Math.random() * 5}s`,
                             backgroundColor: ['#FFD700', '#FF69B4', '#00CED1', '#ffffff'][Math.floor(Math.random() * 4)]
                         }} 
                    />
                ))}
            </div>

            <div className="z-10 text-center mb-8 md:mb-12 mt-8">
                <h1 className="text-5xl md:text-8xl font-black text-yellow-400 uppercase tracking-tighter drop-shadow-[8px_8px_0_rgba(0,0,0,1)] stroke-black mb-4 animate-wobble" style={{WebkitTextStroke: '2px black'}}>
                    PODIUM
                </h1>
            </div>
            ...
            <div className="flex items-end justify-center gap-4 md:gap-8 mb-12 z-10 w-full max-w-5xl px-2 h-[350px] md:h-[450px]">
                {top3[1] ? (
                    <div className="flex flex-col items-center w-1/3 animate-pop" style={{animationDelay: '0.5s'}}>
                        <div className="mb-2 md:mb-4 flex flex-col items-center">
                            <div className="w-16 h-16 md:w-24 md:h-24 rounded-full border-4 border-black bg-gray-300 flex items-center justify-center text-2xl md:text-4xl font-black text-black shadow-hard mb-2">
                                {top3[1].name.charAt(0).toUpperCase()}
                            </div>
                            <span className="font-bold text-white bg-black px-2 py-1 rounded-full text-xs md:text-sm mb-1 truncate max-w-[100px] text-center">{top3[1].name}</span>
                            <span className="font-black text-gray-300 text-lg md:text-2xl">{scores[top3[1].uid]} pts</span>
                        </div>
                        <div className="w-full h-32 md:h-48 bg-gray-300 border-4 border-black rounded-t-2xl shadow-hard flex items-center justify-center relative">
                ...
                {top3[0] ? (
                    <div className="flex flex-col items-center w-1/3 -mt-8 md:-mt-12 animate-pop" style={{animationDelay: '1s'}}>
                        <Crown size={48} className="text-yellow-400 fill-yellow-400 mb-2 md:mb-4 drop-shadow-[4px_4px_0_rgba(0,0,0,1)] animate-bounce"/>
                        <div className="mb-2 md:mb-4 flex flex-col items-center relative">
                            <div className="w-20 h-20 md:w-32 md:h-32 rounded-full border-4 border-black bg-yellow-400 flex items-center justify-center text-4xl md:text-6xl font-black text-black shadow-hard mb-2 z-10">
                                {top3[0].name.charAt(0).toUpperCase()}
                            </div>
                            
                            <span className="font-bold text-white bg-black px-3 py-1 rounded-full text-sm md:text-lg mb-1 truncate max-w-[120px] text-center">{top3[0].name}</span>
                            <span className="font-black text-yellow-400 text-2xl md:text-4xl drop-shadow-md">{scores[top3[0].uid]} pts</span>
                        </div>
                        <div className="w-full h-56 md:h-80 bg-yellow-400 border-4 border-black rounded-t-2xl shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] flex items-center justify-center relative overflow-hidden">
                             <div className="absolute inset-0 bg-white/20 transform rotate-45 translate-y-1/2"></div>
                             <span className="text-8xl font-black text-black/20">1</span>
                        </div>
                    </div>
                ) : null}

                {top3[2] ? (
                    <div className="flex flex-col items-center w-1/3 animate-pop" style={{animationDelay: '0.7s'}}>
                         <div className="mb-2 md:mb-4 flex flex-col items-center">
                            <div className="w-16 h-16 md:w-24 md:h-24 rounded-full border-4 border-black bg-orange-400 flex items-center justify-center text-2xl md:text-4xl font-black text-black shadow-hard mb-2">
                                {top3[2].name.charAt(0).toUpperCase()}
                            </div>
                            <span className="font-bold text-white bg-black px-2 py-1 rounded-full text-xs md:text-sm mb-1 truncate max-w-[100px] text-center">{top3[2].name}</span>
                            <span className="font-black text-orange-400 text-lg md:text-2xl">{scores[top3[2].uid]} pts</span>
                        </div>
                        <div className="w-full h-24 md:h-32 bg-orange-400 border-4 border-black rounded-t-2xl shadow-hard flex items-center justify-center relative">
                             <span className="text-5xl font-black text-black/20">3</span>
                        </div>
                    </div>
                ) : <div className="w-1/3"></div>}
            </div>

            <FunButton onClick={onResults} color="white" className="z-20 text-lg md:text-xl px-8 py-4 animate-pulse mb-8">
                Voir le carnage (Album) <ArrowRight size={24}/>
            </FunButton>
        </div>
    );
}
        ...
        <div className="min-h-screen bg-purple-100 font-sans flex flex-col">
            ...
            <div className="bg-white border-b-4 border-black p-4 shadow-md sticky top-0 z-50 flex flex-col md:flex-row items-center justify-between gap-4">
                 <div className="flex items-center gap-2">
                    <div className="bg-black p-2 rounded-lg"><ImageIcon className="text-white" size={24}/></div>
                    <h1 className="text-2xl font-black uppercase">L'Album Photo</h1>
                 </div>
                 ...
                 <div className="flex gap-2 overflow-x-auto max-w-full pb-2 md:pb-0 scrollbar-hide">
                    {room.players.map(p => (
                        <button
                            key={p.uid}
                            onClick={() => setViewingChainOwner(p.uid)}
                            className={`
                                px-4 py-2 rounded-xl font-bold border-2 border-black whitespace-nowrap transition-all shadow-hard-sm
                                ${viewingChainOwner === p.uid ? 'bg-yellow-400 -translate-y-1 shadow-hard' : 'bg-white hover:bg-gray-100'}
                            `}
                        >
                            {p.name}
                        </button>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-12 bg-pattern">
                <div className="max-w-2xl mx-auto space-y-12 relative">
                    ...
                    <div className="text-center mb-12 relative">
                        <div className="bg-white border-4 border-black p-6 inline-block transform -rotate-2 shadow-hard-lg rounded-sm">
                            <h2 className="text-2xl font-black uppercase text-gray-400">Album de</h2>
                            <h3 className="text-5xl font-black text-purple-600 uppercase">{ownerName}</h3>
                        </div>
                    </div>
                    ...
                    {currentChain.steps.map((step, idx) => (
                        <div key={idx} className="relative z-10 flex gap-6 group">
                            <div className="hidden md:flex flex-col items-center">
                                <div className="w-16 h-16 rounded-full border-4 border-black bg-white flex items-center justify-center text-2xl font-black shadow-hard z-10 relative">
                                    {idx + 1}
                                </div>
                            </div>
                            ...
                            <div className="flex-1">
                                <div className={`
                                    bg-white border-4 border-black p-1 shadow-hard transition-transform hover:-translate-y-1 hover:shadow-hard-lg
                                ...
                                `}>
                                    <div className="bg-gray-100 border-b-4 border-black p-3 flex justify-between items-center">
                                        ...
                                        <div className="flex items-center gap-2">
                                            {step.votes > 0 && (
                                                <div className="bg-yellow-400 border-2 border-black px-2 rounded-full flex items-center gap-1 font-bold text-xs shadow-sm">
                                                    <Star size={12} fill="black"/> {step.votes}
                                                </div>
                                            )}
                                            <span className="text-xs font-bold bg-black text-white px-2 py-1 rounded uppercase">
                                                {step.type === 'TEXT' ? 'ÉCRITURE' : 'DESSIN'}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-6 min-h-[150px] flex items-center justify-center bg-white">
                                        {step.type === 'TEXT' ? (
                                            <p className="text-3xl md:text-4xl font-black text-center uppercase leading-tight text-gray-800" style={{fontFamily: 'cursive'}}>
                                                "{step.content}"
                                            </p>
                                        ) : (
                                            <img src={step.content} alt="Drawing" className="max-w-full border-2 border-gray-200 shadow-inner" />
                                        )}
                                    </div>
                                </div>
                            </div>
            ...
            <div className="p-6 bg-white border-t-4 border-black flex justify-center shadow-[0_-4px_10px_rgba(0,0,0,0.1)]">
                 <FunButton onClick={onRestart} color="purple" icon={Zap}>
                    Nouvelle Partie
                </FunButton>
            </div>
        </div>
